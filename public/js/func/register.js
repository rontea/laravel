...
class PasswordForm {
    ...
    init(){
        ...
        this.validation = {
            ...
            'feedback' : {
                ...
                'confirmPasswordFailed': 'Did not Match Password',
                'generalMessage': 'Password must contain at least one uppercase letter, one special character, one lowercase letter, one number and no whitespace.',
            }
        }
        this._privateEnforceMax.bind(this);
        this._privateMessageShow(this);
    ...
    _privateEnforceMax(){
        this.password.maxLength = this.maxInputLength;
        this.passwordConfirmation.maxLength = this.maxInputLength;
    }

    _privateMessageShow(){
        if(this.feedbackInvalid && this.feedbackValid) {
            this.respondInternal = false;
        }else {
            this.respondInternal = true;
        }
    }
    _privateSetErrorMessage(message){
        if(this.respondInternal){

        }
    }

    _privateSuccessMessage(message){
    ...
    setRepondMessage(validFeedbackId,invalidFeedbackId,message){
    ...
    isPasswordValid(){
        ...
        if(this.valid && this.confirmPasswordState){
            return true;
        }else{
            return false;
        }
...
// usage class expand to password and password confirmation
  ...
  (() => {
    ...
    // Usage
    const userForm = new UserForm('validateForm', 'username',
        'eventChecking', 'respondExist', 'respondValid');
    const passwordForm = new PasswordForm('validateForm',
    'password','password_confirmation');

    passwordForm.setPasswordMin(8);
    passwordForm.setPasswordMax(60);

    // default 5 and 60
    userForm.setLength(5,60);

    const message = {
        min : 'Minimum username should be 5 character long.',
        available : 'Confirm'
    };
    ...
    window.addEventListener('load', function() {
        form.addEventListener("submit", function(event) {
            ...
            // check and proceed
            const isUsernameValid = userForm.isUsernameValid();
            const isPasswordValid = passwordForm.isPasswordValid();

            if(isUsernameValid){
                form.submit();
            }else{
                console.log('Username is not Valid' + isUsernameValid );
            }
        });
    });
